...
def create_stock_from_cli():
    ...
    comment = get_optional_string_from_cli('Comment on the success/failure of the trade')

    # computed fields
    initial_stop = _caculate_initial_stop(entry_price, atr)
    risk_per_stock = _caculate_risk_per_stock(entry_price, initial_stop)
    risk_reward_ratio = _caculate_risk_reward_ratio(risk_per_stock, target_price, entry_price)
    buying_sum = _caculate_buying_sum(position_size, entry_price)
    buying_sum_comission = _caculate_buying_sum_comission(position_size,entry_price, comission)
...
def create_ko_from_cli():
    ...
    # ko only fields
    subscription_ratio = get_number_from_cli('Subscription ratio of the instrument, if relevant')
    knock_out = get_number_from_cli('Knock-out threshold of the instrument, if relevant')
    multiplicator = get_number_from_cli('Multiplicator of the instrument, if relevant')
    strike_price = get_number_from_cli('Strike price of the instrument, if relevant')
    entry_underlying_price = get_number_from_cli('Underlying price at entry, if relevant')
